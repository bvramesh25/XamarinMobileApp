...
namespace VoltPlusPlus.ViewModels
{
   public class ItemsViewModel : BaseViewModel
   {
      ...
      {
         await Shell.Current.GoToAsync(nameof(Views.NewItemPage));
      }
      ...
      async void OnItemSelected(Item item)
      {
         if (item == null)
            return;
         if (item.Text.Equals("Substation Load"))
         {
            await Shell.Current.GoToAsync($"{nameof(SubstationLoadPage)}");
         }
         else if (item.Text.Equals("My Gadgets"))
         {
